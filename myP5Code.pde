...
x += 60;}
  ...
  for(var i = 0; i < 100; i++){
   ...
   moreSnowX.push(random(-10,600));
   moreSnowY.push(random(-100,300));
    }
    
    for(var i = 0; i < moreSnowX.length; i++){
     ellipse(moreSnowX[i], moreSnowY[i], 5, 5);
    moreSnowX[i] += 1;
    }
 
    for(var i = 0; i < 100; i++){
  
   moreSnow1X.push(random(-10,600));
   moreSnow1Y.push(random(-100,300));
    }
    
    for(var i = 0; i < moreSnow1X.length; i++){
    textSize (20);
     text("❆", moreSnow1X[i], moreSnow1Y[i] );
    moreSnow1X[i] += 1;
    }
...
fill(255,0,0)
textSize(100);
text(myPeople[0], 180, 290)
textSize(200);
text(myPeople[1], 280, 270)



//Example Function Call
drawTropicalFish(0, 10);
